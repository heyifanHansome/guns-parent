...
public class UserMgrController extends BaseController {
    ...
    public String userEdit(@PathVariable Integer userId, Model model) {
        ...
        model.addAttribute("cityName", ConstantFactory.me().getCityName(user.getCity()));
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
        ...
        String cityEntity = user.getCity().toString();
        entityWrapper.like("parent_area_tag_id", cityEntity);
        List<AreaTag> areaTags = areaTagService.selectList(entityWrapper);
        if (areaTags.size() > 0) {
            user.setProvince(user.getCity());
            user.setCity(null);
        } else {
            user.setCity(user.getCity());
            AreaTag areaTag = areaTagService.selectById(user.getCity());
            user.setProvince(areaTag.getParentAreaTagId());
        }
