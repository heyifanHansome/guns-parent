...
$(function () {
    var ajax = new $ax(Feng.ctxPath + "/product/getCategoryList", function (data) {
        console.log(data)
        for (var i = 0; i < data.length; i++) {
            var jsonObj = data[i];
            var optionstring = "";
            console.log(jsonObj)
            $("#categoryId").append('<option value="' + jsonObj.categoryId + '">' + jsonObj.name + '</option>');
    ...
    // 初始化头像上传
    var fileServerPathUp = new $WebUpload("picture");
